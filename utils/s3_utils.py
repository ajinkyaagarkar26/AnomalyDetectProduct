...
def download_log_files_from_s3(bucket_name=None, s3_keys=None, local_dir='./datasets', use_credentials=True):
    ...
    if bucket_name is None or s3_keys is None:
        try:
            ...
            try:
                from configs.s3_config import S3_BUCKET_NAME, S3_LOG_FILES, LOCAL_INPUT_DIR
            except ImportError:
    ...
    if use_credentials:
        try:
            # Try different import paths to handle various execution contexts
            try:
                from configs.s3_config import AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION
            except ImportError:
                # Try relative import from parent directory
                import sys
                parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
